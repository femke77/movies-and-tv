...
const FAQPage = () => {
  ...
  const faqData = [
    ...
    },
    {
      ...
      answer:
        "We aren't putting those on you, they come from the servers we link to that allow you to stream content. To some extent it is understandable that content providers need money coming in, but we can't always be sure that the popups are safe or appropriate. If you want to block the popups, the recommendations are as follows: 1) If you are on Safari IOS mobile, you can go into settings for Safari, and under advanced options, then feature flags, you can find a toggle to verify the window.open function. This will stop almost everything (100% all redirects/popups on SOME servers) and some of your taps will appear to do nothing. This approach may interfere with normal browsing on other websites.  2) On Chrome or Firefox desktop, you can add Poper Blocker. This will provide a small popup at the top of the page asking if you want to allow the redirect or not. 3) For Android, we think Firefox + uBlock is a good option. 4) Brave browser is another option in \"Shields Up\" mode but on iOS it can't stop all the pop-ups & redirects and isn't nearly as good as #1. 4) Bingebox devs have created safer servers so try using those. Safe server 1 is the safest. Safe server 2 and 3 are working well now but are being monitored for the addition of redirects. 5) If you are on a desktop/laptop using a mouse, there is an ad/redirect blocking feature that we are implementing soon. Again it will cause some of your clicks to do nothing, and it is not guareteed to work 100% but should reduce the amount significantly. Stand by for this option!",
    },
    {
      question:
